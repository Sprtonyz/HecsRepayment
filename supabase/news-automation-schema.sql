...
-- Market-perception signals are explicitly separate from evidence. They never
-- count toward daily coverage and retain their resolution/calibration trail.
...
revoke all on table public.news_collection_runs, public.news_source_attempts, public.news_articles,
  ...
  , public.news_supporting_contexts, public.news_perception_signals
...
grant select, insert, update, delete on table public.news_collection_runs, public.news_source_attempts,
  ...
  public.monthly_news_reports, public.news_supporting_contexts, public.news_perception_signals to service_role;
